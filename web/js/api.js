const API = {
    ...
    getUserProposals() { return this.get('/api/user/proposals'); },
    getUserProposal(id) { return this.get('/api/user/proposals/' + id); },
    createUserProposal(data) { return this.post('/api/user/proposals', data); },
    updateUserProposal(id, data) { return this.put('/api/user/proposals/' + id, data); },
    getAdminProposals(status) { return this.get('/api/admin/proposals?status=' + (status || 'all')); },
    getAdminProposal(id) { return this.get('/api/admin/proposals/' + id); },
    updateAdminProposal(id, data) { return this.put('/api/admin/proposals/' + id, data); },
    getProposalTestCases(pid) { return this.get('/api/admin/proposals/' + pid + '/testcases'); },
    createProposalTestCase(pid, data) { return this.post('/api/admin/proposals/' + pid + '/testcases', data); },
    updateProposalTestCase(pid, tid, data) { return this.put('/api/admin/proposals/' + pid + '/testcases/' + tid, data); },
    deleteProposalTestCase(pid, tid) { return this.del('/api/admin/proposals/' + pid + '/testcases/' + tid); },
    approveProposal(id, data) { return this.post('/api/admin/proposals/' + id + '/approve', data); },
    rejectProposal(id, data) { return this.post('/api/admin/proposals/' + id + '/reject', data); },
    ...
    banUser(id, is_banned) { return this.put('/api/admin/users/' + id + '/ban', {is_banned}); },
    resetUserPassword(id, password) { return this.put('/api/admin/users/' + id + '/password', {password}); },
    getAdminDiscussions(page, search) { return this.get('/api/admin/discussions?page=' + (page||1) + (search ? '&search=' + encodeURIComponent(search) : '')); },
    getAdminDiscussionReplies(id) { return this.get('/api/admin/discussions/' + id + '/replies'); },
    ...
    getMyRank() { return this.get('/api/leaderboard/my-rank'); },
    getPublicUserProfile(id) { return this.get('/api/users/' + id + '/public'); },
    getConversations() { return this.get('/api/messages/conversations'); },
    createConversation(peer_id) { return this.post('/api/messages/conversations', {peer_id}); },
    getMessages(conv_id, page) { return this.get('/api/messages/conversations/' + conv_id + '?page=' + (page||1)); },
    sendMessage(conv_id, content) { return this.post('/api/messages/conversations/' + conv_id, {content}); },
    getUnreadCount() { return this.get('/api/messages/unread-count'); },
    markRead(conv_id) { return this.post('/api/messages/read/' + conv_id); },
    recallMessage(msg_id) { return this.post('/api/messages/recall/' + msg_id); },
};
